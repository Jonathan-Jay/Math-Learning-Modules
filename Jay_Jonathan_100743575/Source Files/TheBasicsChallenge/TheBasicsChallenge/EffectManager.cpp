...
bool EffectManager::randomized = false;
bool EffectManager::randomized2 = false;
bool EffectManager::randomized3 = false;
bool EffectManager::randomized4 = false;
	...
	if (m_selectable.GetSelected()) {
		if (ImGui::TreeNode("Sepia Effect")) {
			if (m_sepia != -1) {
				...
				if (ImGui::DragFloat("Intensity", &intensity, 0.01f, 0.f, 5.f)) {
					temp->SetIntensity(intensity);
				}

				ImGui::Checkbox("Randomize Intensity", &randomized);
				if (randomized) {
					temp->SetIntensity(rand() % 5);
				}
		...
		if (ImGui::TreeNode("Pixelate Effect")) {
			if (m_pixelate != -1) {
				...
				if (ImGui::DragFloat("Pixel Size", &pixelSize, 0.1f, 1.f, 50.f)) {
					temp->SetPixelSize(pixelSize);
				}

				ImGui::Checkbox("Randomize Pixel Size", &randomized2);
				if (randomized2) {
					temp->SetPixelSize(rand() % 50 + .01f);
				}

				if (ImGui::Button("Big", ImVec2(100.f, 20.f))) {
					temp->SetPixelSize(10000);
				}
		...
		if (ImGui::TreeNode("Grain Effect")) {
			if (m_grain != -1) {
				...
				if (ImGui::DragFloat("Strength", &strength, 1.f, 0.f, 500.f)) {
					temp->SetStrength(strength);
				}

				ImGui::Checkbox("Randomize strength", &randomized3);
				if (randomized3) {
					temp->SetStrength(rand() % 500);
				}
		...
		if (ImGui::TreeNode("Selective Colour Effect")) {
			if (m_selectiveColor != -1) {
				...
				ImGui::Checkbox("Randomize Colours", &randomized4);
				if (randomized4) {
					temp->SetColorRange1(vec3(rand() % 100 / 100.f, rand() % 100 / 100.f, rand() % 100 / 100.f));
				}
