...
inline void from_json(const nlohmann::json& j, Scene& scene)
{
	...
	for (unsigned i = 0; i < numEntities; i++)
	{
		...
		if (reg.get<EntityIdentifier>(entity).GetIsHelloWorld())
		{
			//Sets main player
		...
		if (identity & EntityIdentifier::ScrollCameraBit())
		{
			reg.assign<Scroll>(entity);

			reg.get<Scroll>(entity) = j["Scene"][std::to_string(i)]["ScrollCam"];
	...
	if (scroll) {
		reg.get<Scroll>(EntityIdentifier::MainCamera()).SetCam(&reg.get<Camera>(EntityIdentifier::MainCamera()));
		reg.get<Scroll>(EntityIdentifier::MainCamera()).SetFocus(&reg.get<Transform>(EntityIdentifier::MainPlayer()));
	}
